...
`;
    ...
    const parseMarkdownData = (markdown) => {
        ...
        lines = lines.map(l => l.replace(/\u00A0/g, ' ')); // convert non‑breaking spaces to normal spaces
            ...
            if (currentSection === 'firstArrivals' && currentSubSection) {
                if (line.startsWith('LOCATION')) {
                    ...
                    while (lines[i] && !lines[i].startsWith('Record Holder') && !lines[i].startsWith('###') && !lines[i].startsWith('##')) {
                        const parts = lines[i].split(/\t| {2,}/).filter(p => p.length > 0);
                        if (parts.length === 2) {
                ...
                if (line.startsWith('DAY')) {
                    const headers = line.split(/\t| {2,}/).map(h => h.trim());
